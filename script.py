...
Run Script
----------
python generate_compute_agg_queries.py <path-to-save-directory>
Example:
    python generate_compute_agg_queries.py /home/veloxuser/sandbox/queries
...
expr7 = t.group_by(["a", "b"]).aggregate([t.c.min(), t.c.max()])

# aggregation without groupby
expr8 = t.aggregate([t.c.min(), t.c.max()])

# groupby without aggregate
# NOTE: here after generating the Substrait plan manually remove the 
# measures to just get empty aggregation. Ibis won't generate a Substrait plan
# just with the GroupBy.
expr9 = t.group_by(["a", "b"]).aggregate()
expr1 = t
...
if __name__ == "__main__":
    ...
    if len(args) != 2:
        print("help>>>")
        print("\t python generate_compute_aggregate_queries.py <path-to-save-directory>")
